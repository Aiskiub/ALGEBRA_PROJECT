...
def registrar_usuario():
    ...
    Label(pantalla1, text = "Registro Tradicional Exitoso :)", fg = "green", font = ("Calibri",11)).pack()
    
#-----------------------------funcion pantalla1 INVENTARIO --------------------------------------------------
def mostrar_ventana_inventario():
# Crear almacenes
            # Crear la ventana de la aplicación  
            global ventana_inventario
            ventana_inventario = Toplevel(pantalla)
            ventana_inventario.geometry("300x350")
            ventana_inventario.title("TALLER ALGEBRA ~ Inventario")       #Asignamos el titulo de la pantalla
            #Label(text = "INVENTARIO", bg = "blue", width = "300", height = "2", font = ("Verdana", 13), fg="white").pack() #Asignamos caracteristicas de la ventana
            Label(ventana_inventario, text="Inventario", font=("Verdana", 16)).pack(pady=10)
            # Crear las entradas de texto para el nombre del producto y la cantidad
            #Label(text = "").pack()
            Label(ventana_inventario, text="").pack()
            Label(ventana_inventario, text = "Producto:").pack()
            producto_entry = Entry(ventana_inventario)
            producto_entry.pack()
            Label(ventana_inventario, text = "Cantidad:").pack()
            cantidad_entry = Entry(ventana_inventario)
            cantidad_entry.pack()
            agregar_button = Button(ventana_inventario, text="Agregar producto", command=lambda: almacen1.agregar_producto(producto_entry.get(), int(cantidad_entry.get())))  #BOTONES
            vender_button = Button(ventana_inventario, text="Vender producto", command=lambda: almacen1.vender_producto(producto_entry.get(), int(cantidad_entry.get())))
            mover_button = Button(ventana_inventario, text="Mover producto", command=lambda: almacen1.mover_producto(almacen2, producto_entry.get(), int(cantidad_entry.get())))
            agregar_button.pack()
            vender_button.pack()
            mover_button.pack()
            ...
            #Label(text = "").pack()
            Label(ventana_inventario, text="").pack()
            visualizar_button1 = Button(ventana_inventario, text="Visualizar inventario 1", command=lambda: messagebox.showinfo("Inventario 1", almacen1.visualizar_inventario()))
            visualizar_button2 = Button(ventana_inventario, text="Visualizar inventario 2", command=lambda: messagebox.showinfo("Inventario 2", almacen2.visualizar_inventario()))
...
def registro_facial():
    ...
    while(True):
        ...
        cv2.imshow('Proceso Registro Facial',frame)         #Mostramos el video en pantalla
...
def verificacion_login():
    ...
    if log_usuario in lista_archivos:   #Comparamos los archivos con el que nos interesa
        ...
        if log_contra in verificacion:
            ...
            Button(pantalla2, text = "INGRESAR AL SISTEMA", width = 20, height = 1, command = mostrar_ventana_inventario()).pack()
        else:
...
def pantalla_principal():
    ...
    pantalla = Tk()
    pantalla.geometry("300x350")  #Asignamos el tamaño de la ventana 
    pantalla.title("TALLER ALGEBRA")       #Asignamos el titulo de la pantalla1
    Label(text = "LOGIN", bg = "orange", width = "300", height = "2", font = ("Verdana", 13)).pack() #Asignamos caracteristicas de la ventana
    # Cargar la imagen GIF como ícono
...
#------------------------- Vamos a Crear los Botones ------------------------------------------------------
    ...
    Label(text = "").pack() 
    Label(text = "").pack() 
    Button(text = "Iniciar Sesion", height = "2", width = "30", command = login).pack(pady=5)
    Label(text = "").pack() #Creamos el espacio entre el primer boton y el segundo boton
    Button(text = "Registro", height = "2", width = "30", command = registro).pack(pady=5)
